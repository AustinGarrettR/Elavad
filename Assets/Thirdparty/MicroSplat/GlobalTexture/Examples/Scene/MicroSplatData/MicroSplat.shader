...
void PrepareStochasticUVs(float scale, float2 uv, out float2 uv1, out float2 uv2, out float2 uv3, out half3 weights)
{
   ...
   weights = half3(w1, w2, w3);
      ...
      // pertex needs to be applied individually
      void GlobalTintTexturePerTex(inout RawSamples s, Config config, float camDist, float slopeFilter)
      {
      ...
      // no per tex is faster, just final value.
      void GlobalTintTexture(inout half3 albedo, Config config, float camDist, float slopeFilter)
      {
      ...
      // pertex needs to be applied individually
      void GlobalSpecularTexturePerTex(inout RawSamples s, Config config, float camDist, float slopeFilter)
      {
      #if _USESPECULARWORKFLOW && _GLOBALSPECULAR
         float2 uv = (config.uv + _GlobalSpecularUVScale.zw) * _GlobalSpecularUVScale.xy;
         half4 tex = UNITY_SAMPLE_TEX2D_SAMPLER(_GlobalSpecularTex, _Diffuse, uv);
         COUNTSAMPLE
         float str = _GlobalTextureParams.x * tex.a;
      ...
      // no per tex is faster, just final value.
      void GlobalSpecularTexture(inout half3 specular, Config config, float camDist, float slopeFilter)
      {
       #if _USESPECULARWORKFLOW && _GLOBALSPECULAR
       ...
       // pertex needs to be applied individually
      void GlobalNormalTexturePerTex(inout RawSamples s, Config config, float camDist, float slopeFilter)
      {
            ...
            half4 rawNormal = UNITY_SAMPLE_TEX2D_SAMPLER(_GlobalNormalTex, _Diffuse, uv);
            COUNTSAMPLE
      ...
      // no per tex is faster, just final value.
      void GlobalNormalTexture(inout half4 normSAO, Config config, float camDist, float slopeFilter)
      {
         #if !_PERTEXGLOBALNORMALSTRENGTH
            float2 uv = (config.uv + _GlobalNormalUVScale.zw) * _GlobalNormalUVScale.xy;
            float str = _GlobalTextureParams.y;
      ...
      void GlobalSAOMTexturePerTex(inout RawSamples s, Config config, float camDist, float slopeFilter)
      {
      ...
      // no per tex is faster, just final value.
      void GlobalSAOMTexture(inout half4 nsao, inout half4 emisMetal, Config config, float camDist, float slopeFilter)
      {
      ...
      #if _GLOBALEMIS
      // pertex needs to be applied individually
      void GlobalEmisTexturePerTex(inout RawSamples s, Config config, float camDist, float slopeFilter)
      {
      ...
      // no per tex is faster, just final value.
      void GlobalEmisTexture(inout half4 emisMetal, Config config, float camDist, float slopeFilter)
      {
...
ENDCG
   ...
   Dependency "BaseMapShader" = "Hidden/MicroSplat/Example_GlobalTexturing_Base799131421"
