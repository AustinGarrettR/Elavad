...
public class StreamManager : MonoBehaviour 
{
   ...
   static Vector2 WorldToTerrain(MicroSplatObject ter, Vector3 point, Texture splatControl)
   {
      Bounds b = ter.GetBounds ();
      point = ter.transform.worldToLocalMatrix.MultiplyPoint(point);
      float x = (point.x / b.size.x) * splatControl.width;
      float z = (point.z / b.size.z) * splatControl.height;
	...
	void Update () 
   {
      ...
      {
         var e = emitters[i];
         Vector2 ter = WorldToTerrain(msObject, e.transform.position, buffer0);
         if (ter.x >= 0 && ter.x < buffer0.width && ter.y >= 0 && ter.y < buffer0.width)
         {
            ...
            Vector2 endPoint = WorldToTerrain(msObject, pos, buffer0);
      ...
      {
         var c = colliders[i];
         Vector2 ter = WorldToTerrain(msObject, c.transform.position, buffer0);
      ...
      msObject.matInstance.SetTexture("_DynamicStreamControl", currentBuffer);
